...
@RestController
@Validated
public class CustomerAccountInitiateApi {

    @Autowired
    private CustomerAccountInitiateService customerAccountInitiateService;
    private final Logger logger = LoggerFactory.getLogger(CustomerAccountInitiateApi.class);
    ...
    @ApiOperation(value = "Customer Account Initiate",
        nickname = "Add Customer Account",
    ...
    @ApiResponses(value = {
        ...
        @ApiResponse(responseCode = "500", description = "Internal Server Error",
            content = @Content(mediaType = "application/json; charset&#x3D;utf-8",
                schema = @Schema(implementation = ErrorResponse.class)))})
    @PostMapping(value = "/PartyReference/NewSavingsAccount/Initiate",
        produces = {"application/json", "application/json; charset=utf-8", "application/jose+jwe"},
        consumes = {"application/json"})
    public Mono<ResponseEntity<AccountResponse>> custAcctInitiateCall(
        ...
        @Parameter(in = ParameterIn.DEFAULT, description = "", required = true,
            schema = @Schema())
        @Valid @RequestBody(required = true) final AccountRequest accountRequest
    ) {
        ...
        final Mono<AccountResponse> responseMono = customerAccountInitiateService.initiateCustomerAccount(
            accountRequest, ServletUriComponentsBuilder.fromCurrentRequest());
