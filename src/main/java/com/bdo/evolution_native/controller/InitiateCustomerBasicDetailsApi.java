...
@RestController
@Validated
public class InitiateCustomerBasicDetailsApi {

    @Autowired
    private InitiateCustomerBasicDetailsService initiateCustomerBasicDetailsService;
    private final Logger logger = LoggerFactory.getLogger(InitiateCustomerBasicDetailsApi.class);
    ...
    @ApiOperation(value = "Initiate Customer Basic Details Service",
        nickname = "Add Customer Basic Details",
        response = AddCustomerResponse.class)
    @ApiResponses(value = {
        @ApiResponse(responseCode = "201", description = "Created",
            content = @Content(mediaType = "application/json",
                schema = @Schema(implementation = AddCustomerResponse.class))),
        ...
        @ApiResponse(responseCode = "500", description = "Internal Server Error",
            content = @Content(mediaType = "application/json; charset&#x3D;utf-8",
                schema = @Schema(implementation = ErrorResponse.class)))})
    @PostMapping(value = "/PartyReference/CustomerProfile/Initiate",
        produces = {"application/json", "application/json; charset=utf-8", "application/jose+jwe"},
        consumes = {"application/json"})
    public Mono<ResponseEntity<AddCustomerResponse>> empDetailsInitiateCall(
        ...
        @Parameter(in = ParameterIn.DEFAULT, description = "", required = true,
            schema = @Schema())
        @Valid @RequestBody(required = true) final AddCustomerRequest addCustomerRequest
    ) {
        ...
        final Mono<AddCustomerResponse> responseMono = initiateCustomerBasicDetailsService.initiateCustomer(
            addCustomerRequest, ServletUriComponentsBuilder.fromCurrentRequest());
